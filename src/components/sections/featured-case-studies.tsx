...
export function FeaturedCaseStudies() {
  return (
    <section className="py-20 md:py-24 bg-brand-warm relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,_rgba(200,147,42,0.04),transparent_50%)]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {studies.map((study, index) => (
            ...
            >
              <Link
                href={study.href}
                className="group block h-full bg-white rounded-xl border border-border p-8 hover:border-brand-gold/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 relative overflow-hidden"
              >
                ...
                <div className="relative">
                  <div className="inline-flex items-center gap-1.5 bg-brand-gold/10 rounded-full px-3 py-1 mb-4">
                    <span className="w-1.5 h-1.5 rounded-full bg-brand-gold" />
                    <span className="text-brand-gold text-xs font-medium">{study.service}</span>
                  </div>
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-brand-cream to-brand-cream/80 border border-brand-gold/20 mb-5 group-hover:from-brand-gold/20 group-hover:scale-110 transition-all duration-500">
                    <study.icon className="h-6 w-6 text-brand-gold" />
                  </div>
                  <h3 className="font-heading text-lg font-bold text-secondary mb-3 group-hover:text-brand-gold transition-colors duration-500">
                    {study.title}
                  </h3>
                  <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                    {study.description}
                  </p>
                  <div className="bg-brand-gold/5 border border-brand-gold/10 rounded-lg px-4 py-2.5 mb-4">
